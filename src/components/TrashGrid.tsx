...
export const TrashGrid: React.FC = () => {
    ...
    if (deletedNotes.length === 0) {
        return (
            <div className="w-full h-screen flex flex-col items-center justify-center bg-gray-50/90 backdrop-blur-sm animate-in fade-in duration-300">
                <div className="text-gray-300 mb-4">
                    <Trash2 size={64} strokeWidth={1} />
                </div>
                <h2 className="text-xl font-medium text-gray-400">废纸篓是空的</h2>
                <p className="text-sm text-gray-400 mt-2">这里没有已删除的便签</p>
                <button
                    onClick={() => setViewMode('BOARD')}
                    className="mt-6 px-6 py-2.5 bg-white border border-gray-200 text-gray-700 rounded-lg hover:bg-gray-50 transition-colors text-sm font-medium shadow-sm"
                >
                    返回看板
                </button>
            </div>
    ...
    return (
        <div className="w-full h-screen bg-gray-50/95 backdrop-blur-sm overflow-y-auto animate-in fade-in duration-300 z-40 relative">
            
            {/* Header with Drag Region */}
            <div 
                data-tauri-drag-region 
                className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-200 px-8 py-4 flex items-center justify-between"
            >
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-xl font-bold text-gray-800">废纸篓</h1>
                        <p className="text-xs text-gray-500">{deletedNotes.length} 个已删除的便签</p>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button 
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-red-50 text-red-600 border border-red-100 rounded-lg hover:bg-red-100 transition-colors text-sm font-medium shadow-sm"
                    >
                        <Trash2 size={16} />
                        清空废纸篓
                    </button>
                ...
                {deletedNotes.map(note => (
                    <div key={note.id} className="relative group flex flex-col">
                        ...
                        <div className="relative">
                            ...
                            {/* Overlay Mask */}
                            <div className="absolute inset-0 bg-white/10 group-hover:bg-white/0 transition-colors pointer-events-none rounded-2xl" />
                        </div>
