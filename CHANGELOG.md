...
### 🐛 问题修复 (Bug Fixes)
*   **便签头部交互**：拆分了便签头部中的拖拽把手、折叠触发区和按钮区域，减少误触拖拽、误触折叠与头部操作互相干扰的问题。
*   **折叠态复制按钮**：修复了便签折叠后仍显示复制按钮的问题，让折叠态头部反馈更一致。
*   **画布空白命中**：修复了便签头部交互可能被误判为空白画布操作的问题，避免双击头部误新建便签、点击头部误清空选择。

### 🚀 优化 (Optimizations)
*   **头部显隐一致性**：收敛了标题输入、头部控件与折叠标题的显示规则，提升悬浮、编辑与折叠切换时的状态一致性。
*   **回归保障**：新增 NoteCard 与 Canvas 交互测试，覆盖头部命中边界、折叠态按钮显示与空白画布判定，降低后续交互改动的回归风险。

## [v1.2.1] - 2026-04-11

### 🐛 问题修复 (Bug Fixes)
*   **MiniMap 看板隔离**：修复了小地图会混入其他看板便签的问题。现在 MiniMap 仅显示当前看板且未删除的便签，避免跨看板概览干扰。
*   **MiniMap 边界计算**：修复了小地图缩放边界会被其他看板内容放大的问题。切换看板后，当前视口与概览范围的对应关系更加准确。
*   **MiniMap 隐藏态交互**：修复了小地图隐藏时仍可能占据右下角交互区域的问题，避免误挡画布点击。

### 🚀 优化 (Optimizations)
*   **空看板与切板稳定性**：优化了空看板、看板切换场景下的小地图视口显示逻辑，保证概览组件在无便签时依然保持稳定反馈。
*   **回归保障**：新增 MiniMap 组件测试，覆盖当前看板过滤、切换看板更新与空看板场景，降低后续小地图改动的回归风险。
