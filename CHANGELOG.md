...
### 🚀 优化 (Optimizations)
* **搜索索引下沉**
  > 搜索索引构建、匹配、高亮计算从主线程迁移到 Web Worker，
  > 1000+ 便签场景下输入搜索词不再阻塞画布交互。
...
### 🐛 问题修复 (Bug Fixes)
* **CI 测试 OOM 修复**
  > vitest 测试进程内存溢出：改用 `threads` pool 替代 `forks`，限定 `vi.useFakeTimers()` 范围避免劫持 `performance`/`queueMicrotask`，移除 vitest 4 废弃的 `poolOptions` 配置。
