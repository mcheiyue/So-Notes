...
2. **版本摘要**：每个版本标题下方必须先写一段简短摘要，说明本版本聚焦的问题、能力或发布目的。
3. **分类标题顺序**：按下列顺序组织；没有内容的分类直接省略。
   ...
   - `### 🔧 内部改进 (Internal)`
   - `### 🧹 清理 (Cleanup)`
4. **条目结构**：采用“标题 → 用户影响 → 必要细节”的三层颗粒度。
   - 标题使用 `* **面向用户或维护者的短标题**`，避免直接使用文件名或提交式描述。
   - 引用块 `>` 说明用户可感知的变化、行为差异或维护价值。
   - 重要条目可补充 1-4 条细节列表，用于说明入口、范围、边界、兼容性或定量收益。
   - 普通修复和清理不强制补充细节列表，避免把发布说明写成 commit log。
5. **表达原则**：标题面向结果，说明面向影响，细节面向边界；技术实现只在有助于理解影响或维护风险时出现。
...
本版本聚焦数据安全与窗口恢复能力，修复缓存覆盖问题，并补充托盘重置窗口入口。

### ✨ 新特性 (Features)

* **新增托盘重置窗口**
  > 托盘右键菜单新增“重置窗口”按钮，可一键将窗口大小重置并归位到屏幕右下角。
